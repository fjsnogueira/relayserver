...
namespace Thinktecture.Relay.Server.Diagnostics
{
    internal class TraceManager : ITraceManager
    {
        ...
        public TraceManager(ITraceRepository traceRepository, ITraceFileWriter traceFileWriter, ITraceFileReader traceFileReader,
        ...
        public void Trace(IOnPremiseConnectorRequest onPremiseConnectorRequest, IOnPremiseTargetReponse onPremiseTargetReponse,
            Guid traceConfigurationId)
        {
            try
            {
                ...
                var filenamePrefix = String.Format("{0}-{1}",
                    Path.Combine(_configuration.TraceFileDirectory, traceConfigurationId.ToString()), DateTime.Now.Ticks);
        ...
        private async Task<Trace> GetTraceAsync(string filePrefix)
        {
            ...
            var onPremiseTargetHeaders =
                await _traceFileReader.ReadHeaderFileAsync(filePrefixWithDirectory + OnPremiseTargetHeaderExtension);
