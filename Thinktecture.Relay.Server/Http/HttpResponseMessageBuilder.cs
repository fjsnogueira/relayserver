            ...
            _contentHeaderTransformation = new Dictionary<string, Action<HttpContent, string>>()
            {
                ...
                {"Content-Type", (r, v) => r.Headers.ContentType = MediaTypeHeaderValue.Parse(v)},
                {
                    ...
                    (r, v) =>
                        ...
                        (v == "-1"
                            ? (DateTimeOffset?) null
                            : new DateTimeOffset(DateTime.ParseExact(v, "R", CultureInfo.InvariantCulture)))
                },
                {
                    "Last-Modified",
                    (r, v) => r.Headers.LastModified = new DateTimeOffset(DateTime.ParseExact(v, "R", CultureInfo.InvariantCulture))
                }
            };
        }
